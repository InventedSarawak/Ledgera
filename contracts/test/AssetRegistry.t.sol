...
contract AssetRegistryTest is Test {
    ...
    function testCreatedTokenIsERC1155() public {
        vm.prank(creator);
        address tokenAddress = assetRegistry.createAsset('ERC1155 Token', 'E15');

        AssetToken token = AssetToken(tokenAddress);

        // Verify it supports ERC-1155 by calling ERC-1155-specific methods
        // Mint some supply and verify ERC-1155 balance tracking
        vm.prank(creator);
        token.mintInitialSupply(creator, 10_000);

        assertEq(token.balanceOf(creator, 0), 10_000);
    }
}
